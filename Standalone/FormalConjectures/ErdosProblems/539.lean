...
The principal version includes `sorry` for an unknown function. On the other hand, the best
...
theorem erdos_539 :
    (fun n ↦ (cofactorThreshold n : ℝ)) =Θ[atTop] (sorry : ℕ → ℝ) := by
  sorry
...
theorem erdos_539.variants.limit :
    atTop.Tendsto (fun n ↦ Real.log (cofactorThreshold n) / Real.log n) sorry := by
  sorry
