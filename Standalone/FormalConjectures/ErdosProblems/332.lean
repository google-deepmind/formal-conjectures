...
This is formalised here using the `sorry` mechanism. In order to solve this problem one
...
-/
theorem erdos_332 (A : Set ℕ) : (sorry : Set ℕ → Prop) A → HasBoundedGaps (D_A A) := by
  sorry
