...
Lean code uses [highlight.js](https://highlightjs.org/) with the
[Lean community grammar](https://github.com/leanprover-community/highlightjs-lean)
and Atom One Light theme. The theorem and full annotated source pages load pinned
versions from esm.sh and jsDelivr. The shared highlighter preserves Verso's hover targets and links, as well as comment
indentation and line breaks. If the library cannot load, the source remains readable.

`fix_literate_html.py` copies the shared highlighting assets into the literate
output and adds them to each source page. Run it again after changing
`src/js/lean-highlight.js` or `src/css/lean-highlight.css` to refresh source previews.
