...
(function () {
  ...
  function login() {
    // Route OAuth callback through the shared proxy, which bounces back to
    // the user's site with the code. This lets one registered callback URL
    // work for all forks.
    // Pass the return URL in the OAuth state parameter. GitHub passes it
    // through to the callback unchanged. The redirect_uri must exactly match
    // the registered callback URL (no extra query params).
    const redirectUri = `${WORKER_URL}/oauth/callback`;
    window.location.href = `https://github.com/login/oauth/authorize?client_id=${encodeURIComponent(GH_CLIENT_ID)}&redirect_uri=${encodeURIComponent(redirectUri)}&state=${encodeURIComponent(window.location.href)}`;
  }
  ...
  async function handleOAuthCallback() {
    ...
    try {
      ...
      console.log('Token exchange response keys:', Object.keys(tokenData), 'token_type:', tokenData.token_type);
  ...
  async function graphql(query, variables, token) {
    ...
    if (!resp.ok) {
      ...
      console.error(`GraphQL ${resp.status}: ${text}`);
  ...
  // Discussion helpers
  // ---------------------------------------------------------------------------
  const DISCUSSION_CATEGORY_NAME = 'Problems';
  ...
  async function fetchCategoryId(token) {
    ...
    const match = cats.find(c => c.name === DISCUSSION_CATEGORY_NAME);
    if (!match) throw new Error(`Discussion category "${DISCUSSION_CATEGORY_NAME}" not found. Create it in the repository settings under Discussions.`);
    categoryIdCache = match.id;
  ...
  // In-flight discussion creation promises to prevent concurrent duplicates
  ...
  async function ensureDiscussion(theoremName) {
    ...
    // If another call is already creating this discussion, wait for it
    if (pendingDiscussions.has(theoremName)) {
      return pendingDiscussions.get(theoremName);
    }

    const promise = doEnsureDiscussion(theoremName);
    pendingDiscussions.set(theoremName, promise);
    try {
      return await promise;
    } finally {
      pendingDiscussions.delete(theoremName);
    }
  }

  async function doEnsureDiscussion(theoremName) {
    const { token } = getUser();
    if (!token) throw new Error('Not authenticated');

    // List discussions directly (avoids search index lag that causes duplicates)
    ...
    while (hasNextPage) {
      ...
      const match = discussions.nodes.find(n => n.title === theoremName);
      if (match) {
  ...
  async function fetchAllVotes() {
    ...
    // Always fetch aggregates from the proxy (uses GitHub App installation token, cached)
    try {
  ...
  function renderTruthWidget(theoremName, container) {
    ...
    if (!isLoggedIn()) {
      container.innerHTML = `
        <button class="truth-btn" title="Sign in to predict" aria-label="Sign in to predict">
          <span class="truth-btn__label">True</span>
          <span class="truth-btn__count">${thumbsUp}</span>
        </button>
        <button class="truth-btn" title="Sign in to predict" aria-label="Sign in to predict">
          <span class="truth-btn__label">False</span>
      ...
      for (const btn of container.querySelectorAll('.truth-btn')) {
        btn.addEventListener('click', function () { loginWithConsent(); });
      }
      return;
    }
    ...
    upBtn.innerHTML = `<span class="truth-btn__label">True</span><span class="truth-btn__count">${thumbsUp}</span>`;
    ...
    downBtn.innerHTML = `<span class="truth-btn__label">False</span><span class="truth-btn__count">${thumbsDown}</span>`;
  ...
  function renderCardTruth(theoremName) {
    ...
    return `<span class="theorem-card__truth" title="Predictions: ${thumbsUp} true, ${thumbsDown} false">T ${thumbsUp} F ${thumbsDown}</span>`;
  }
