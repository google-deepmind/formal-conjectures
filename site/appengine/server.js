...
// ---------------------------------------------------------------------------
// Repo validation — only serve repos that are formal-conjectures or forks of it
// ---------------------------------------------------------------------------
const UPSTREAM_FULL_NAME = 'google-deepmind/formal-conjectures';
const validatedRepos = new Map(); // "owner/repo" -> true/false

async function isAllowedRepo(owner, repo) {
  const key = `${owner}/${repo}`;
  if (validatedRepos.has(key)) return validatedRepos.get(key);

  if (key === UPSTREAM_FULL_NAME) {
    validatedRepos.set(key, true);
    return true;
  }

  // Check if the repo is a fork of the upstream
  try {
    ...
    if (!resp.ok) {
      validatedRepos.set(key, false);
      return false;
    }
    const data = await resp.json();
    const allowed = data.fork && data.source && data.source.full_name === UPSTREAM_FULL_NAME;
    validatedRepos.set(key, allowed);
    return allowed;
  } catch (e) {
    return false;
  }
...
// GET /oauth/callback — bounce OAuth code back to the user's site.
// GitHub passes the return URL through the OAuth `state` parameter.
app.get('/oauth/callback', (req, res) => {
  ...
  // Only redirect to *.github.io or localhost (prevent open redirect)
  let target;
  try {
    target = new URL(returnTo);
  } catch (e) {
    return res.status(400).send('Invalid return URL in state parameter');
  }
  const isGitHubPages = /^[a-zA-Z0-9_-]+\.github\.io$/.test(target.hostname);
  const isLocalhost = target.hostname === 'localhost' || target.hostname === '127.0.0.1';
  if (!isGitHubPages && !isLocalhost) {
    return res.status(403).send('Return URL must be a *.github.io or localhost URL');
  }
...
app.get('/discussions', async (req, res) => {
  ...
  if (!await isAllowedRepo(owner, repo)) {
    return res.status(403).json({ error: 'Repository is not formal-conjectures or a fork of it' });
  }
