...
elab "#recordAMSSubjects" : command => do
  let env ← Lean.getEnv
...
elab "#AMS" : command => do
  let some ci := (← Lean.getEnv).find? (.mkSimple "amsSubjects")
    | throwError "`amsSubjects` is missing; `#recordAMSSubjects` should have declared it"
