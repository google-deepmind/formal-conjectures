...
`U` containing it gives a ring homomorphism `Γ(X.left, U) ⟶ CommRingCat.of R`. Over an affine open
this identifies the `R`-points with a subspace of that hom set, which carries Mathlib's topology of
pointwise convergence. The analytic topology is the topology glued from these affine charts. It
exists as soon as multiplication on `R` is continuous and the units of `R` are open, so it applies
to `ℝ`, to a `p`-adic field, and to `ℤ_p` alike.
...
ring homomorphism `Γ(X.left, U) ⟶ CommRingCat.of R`.
...
@[simp]
lemma underlying_map {Y : Over (Spec (CommRingCat.of R))} (f : X ⟶ Y) (z : Point R X) :
    (map f z).underlying = f.left z.underlying := rfl

/-- The stalk homomorphism of the image of an `R`-point is obtained functorially. -/
lemma stalkHom_map {Y : Over (Spec (CommRingCat.of R))} (f : X ⟶ Y) (z : Point R X) :
...
lemma mem_overOpen_map_iff {Y : Over (Spec (CommRingCat.of R))} (f : X ⟶ Y)
...
lemma evaluate_map {Y : Over (Spec (CommRingCat.of R))} (f : X ⟶ Y)
...
convergence, pulled back along `evaluationHom` from Mathlib's topology on
`Γ(X.left, U) ⟶ CommRingCat.of R`. -/
@[instance_reducible]
...
which carries Mathlib's topology of pointwise convergence on `Γ(X.left, U) ⟶ CommRingCat.of R`.
...
out by an open set of ring homomorphisms `Γ(X.left, U) ⟶ CommRingCat.of R`. -/
lemma isOpen_analyticTopology_iff {W : Set (Point R X)} :
    IsOpen W ↔ ∀ U : X.left.affineOpens,
      ∃ O : Set (Γ(X.left, U.1) ⟶ (CommRingCat.of R)), IsOpen O ∧ evaluationHom U.1 ⁻¹' O = Subtype.val ⁻¹' W := by
...
lemma isOpen_overOpen_inter_preimage (U : X.left.Opens) (s : Γ(X.left, U))
  ...
  refine ⟨⋃ (g : Γ(X.left, W)) (k : ℕ) (a : Γ(X.left, W)) (_ : X.left.basicOpen g ≤ U ⊓ W)
      ...
      {φ : Γ(X.left, W) ⟶ (CommRingCat.of R) | IsUnit (φ g)} ∩
        (fun φ : Γ(X.left, W) ⟶ (CommRingCat.of R) ↦ Ring.inverse (φ g ^ k) * φ a) ⁻¹' V,
      ?_, ?_⟩
  · refine isOpen_iUnion fun g ↦ isOpen_iUnion fun k ↦ isOpen_iUnion fun a ↦
      isOpen_iUnion fun _ ↦ isOpen_iUnion fun _ ↦ ?_
    have hg : Continuous fun φ : Γ(X.left, W) ⟶ (CommRingCat.of R) ↦ φ g :=
      CommRingCat.HomTopology.continuous_apply (R := (CommRingCat.of R)) g
    have ha : Continuous fun φ : Γ(X.left, W) ⟶ (CommRingCat.of R) ↦ φ a :=
      CommRingCat.HomTopology.continuous_apply (R := (CommRingCat.of R)) a
    have hinv : ContinuousOn (fun φ : Γ(X.left, W) ⟶ (CommRingCat.of R) ↦ Ring.inverse (φ g ^ k))
        {φ : Γ(X.left, W) ⟶ (CommRingCat.of R) | IsUnit (φ g)} :=
...
lemma analyticTopology_eq_generateFrom :
  ...
  · intro W hW
    ...
    obtain ⟨O', hO', rfl⟩ :=
      (CommRingCat.HomTopology.isEmbedding_hom (CommRingCat.of R) Γ(X.left, U)).isInducing.isOpen_iff.mp hO
    ...
    · rintro y ⟨hyU, hyu⟩
      ...
      have hmem : (⟨y, hyU⟩ : OverOpen (X := X) U) ∈
          evaluationHom U ⁻¹' ((fun f : Γ(X.left, U) ⟶ (CommRingCat.of R) ↦ ⇑f.hom) ⁻¹' O') := hy
      rwa [hWO] at hmem
...
lemma continuous_map {Y : Over (Spec (CommRingCat.of R))} (f : X ⟶ Y) :
...
@[simp]
lemma isoMapHomeomorph_apply {Y : Over (Spec (CommRingCat.of R))} (e : X ≅ Y)
    (z : Point R X) :
...
variable {K : Type} [Field K] {X : Over (Spec (CommRingCat.of K))}
...
noncomputable def residueData (z : Point K X) :
    Σ x : X.left, X.left.residueField x ⟶ (CommRingCat.of K) :=
...
lemma residueData_map {Y : Over (Spec (CommRingCat.of K))} (f : X ⟶ Y) (z : Point K X) :
    (map f z).residueData =
...
lemma residueData_map_snd {Y : Over (Spec (CommRingCat.of K))} (f : X ⟶ Y) (z : Point K X) :
...
noncomputable def complexAnalytification :
    CategoryTheory.Functor (Over (Spec (CommRingCat.of ℂ))) TopCat where
