...
Projective space over a scheme is constructed as a pullback from `Proj` in
`FormalConjecturesForMathlib.AlgebraicGeometry.ProjectiveSpace`. This file proves its structure
morphism proper for finitely many homogeneous coordinates, and packages a closed embedding into
finite-dimensional projective space as an explicit projective presentation.
