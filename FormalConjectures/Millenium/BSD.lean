...
the local minimal discriminant ideals `v.asIdeal ^ W.localMinimalDiscriminantExponent v` over
...
noncomputable def minimalDiscriminantIdeal {K : Type*} [Field K] [NumberField K]
  ...
  ∏ᶠ v : HeightOneSpectrum (𝓞 K), v.asIdeal ^ W.localMinimalDiscriminantExponent v
