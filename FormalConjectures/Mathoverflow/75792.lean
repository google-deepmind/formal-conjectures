...
-- The equation compiler emits `Reachable.decide._unary`, whose reducibility Lean warns about;
-- the instance is deliberately semireducible, so the warning is switched off here.
