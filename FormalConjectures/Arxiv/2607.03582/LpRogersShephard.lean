...
noncomputable def supportFunction (K : Set E) (u : E) : ℝ := ⨆ x : K, inner ℝ (x : E) u
...
existence argument. -/
noncomputable def lpSum (p : ℝ) (K L : Set E) : Set E :=
  {x | ∀ u : E, inner ℝ x u ≤ (supportFunction K u ^ p + supportFunction L u ^ p) ^ (1 / p)}

/-- `K` has a centre of symmetry: some `c` with `K` invariant under reflection in `c`. -/
def HasCentreOfSymmetry (K : Set E) : Prop := ∃ c : E, ∀ x, x ∈ K ↔ (2 • c - x) ∈ K
...
def IsParallelogramAtOrigin (K : Set E) : Prop :=
  ∃ a b : E, LinearIndependent ℝ ![a, b] ∧ K = convexHull ℝ {0, a, b, a + b}
...
theorem volume_lpSum_le (K : Set E) (hK : Convex ℝ K) (hKc : IsCompact K)
    (hKi : (interior K).Nonempty) (hsym : HasCentreOfSymmetry K) (h0 : (0 : E) ∈ K)
...
-/
@[category research solved, AMS 52]
theorem volume_lpSum_eq_of_isParallelogramAtOrigin (K : Set E)
...
theorem isParallelogramAtOrigin_of_volume_lpSum_eq :
    answer(sorry) ↔ ∀ (K : Set E), Convex ℝ K → IsCompact K → (interior K).Nonempty →
      HasCentreOfSymmetry K → (0 : E) ∈ K → ∀ p q : ℝ, 1 < p → 1 / p + 1 / q = 1 →
        volume (lpSum p K (-K)) = ENNReal.ofReal (rsConstant q) * volume K →
          IsParallelogramAtOrigin K := by
  sorry

/-- At `p = 2` the conjugate is also `2`, and `Γ(3/2)^2 / Γ(2) = π/4`, so the constant is
`π/2 + 2`. -/
@[category API, AMS 52]
