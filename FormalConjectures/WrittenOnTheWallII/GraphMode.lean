...
For a graph `G`, the **degree count** `countDegreeK G d` is the number of vertices
...
In a connected graph every vertex has degree ≥ 1, so `countDegreeK G 0 = 0`.
Because some vertex has degree ≥ 1, there exists `d ≥ 1` with
`countDegreeK G d > 0 = countDegreeK G 0`.  Hence the modal degree value must be ≥ 1.
-/
...
/-- In `K₃`, every vertex has degree 2.  So `countDegreeK K₃ 2 = 3`. -/
@[category test, AMS 5]
example : countDegreeK (⊤ : SimpleGraph (Fin 3)) 2 = 3 := by
  unfold countDegreeK
  sorry
...
@[category test, AMS 5]
example (G : SimpleGraph (Fin 5)) [DecidableRel G.Adj] : 0 ≤ modeDegreeMin G :=
  Nat.zero_le _

/-- `maxDegreeCount` is nonneg. -/
@[category test, AMS 5]
example (G : SimpleGraph (Fin 3)) [DecidableRel G.Adj] : 0 ≤ maxDegreeCount G :=
  Nat.zero_le _
