...
/-- **Randomized ETH** (Liu–Chen, Conjecture 5), in the time-budget-width
...
$O(\log(\max(L,T)+2))$ for the full time budget $T$, permitting exponentially
many addressable cells when $T$ is exponential in $n$. -/
