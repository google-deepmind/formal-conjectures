...
theorem erdos_1083 : answer(sorry) ↔ ∀ d : ℕ, 3 ≤ d →
    Tendsto (fun n : ℕ => Real.log (Module.minimalDistinctDistances d n) / Real.log n)
      atTop (𝓝 (2 / (d : ℝ))) := by
  sorry
...
theorem variants.grid_upper_bound (d : ℕ) (hd : 3 ≤ d) :
    (Module.minimalDistinctDistances d · : ℕ → ℝ) =O[atTop] (· ^ (2 / (d : ℝ)) : ℕ → ℝ) := by
  sorry
...
theorem variants.erdos_lower_bound (d : ℕ) (hd : 3 ≤ d) :
    (· ^ (1 / (d : ℝ)) : ℕ → ℝ) =O[atTop] (Module.minimalDistinctDistances d · : ℕ → ℝ) := by
  sorry
...
theorem variants.lower_bound_cegsw :
    (· ^ (1 / 2 : ℝ) : ℕ → ℝ) =O[atTop] (Module.minimalDistinctDistances 3 · : ℕ → ℝ) := by
  sorry
...
theorem variants.lower_bound_apst (d : ℕ) (hd : 3 ≤ d) :
    ∀ ε > (0 : ℝ), ∀ᶠ n : ℕ in atTop,
      (n : ℝ) ^ (1 / ((d : ℝ) - 90 / 77) - ε) ≤ Module.minimalDistinctDistances d n := by
  sorry
...
theorem variants.lower_bound_solymosi_vu (d : ℕ) (hd : 3 ≤ d) :
    ((· : ℕ → ℝ) ^ (2 / (d : ℝ) - 2 / ((d : ℝ) * ((d : ℝ) + 2)))) =O[atTop]
      (Module.minimalDistinctDistances d · : ℕ → ℝ) := by
  sorry
...
theorem variants.lower_bound_solymosi_vu_dim_three :
    (· ^ (0.5643 : ℝ) : ℕ → ℝ) =O[atTop] ((Module.minimalDistinctDistances 3 ·) : _ → ℝ) := by
  sorry
