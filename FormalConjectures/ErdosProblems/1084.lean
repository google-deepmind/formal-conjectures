...
theorem erdos_1084_upper_d1 : f 1 n = n - 1 := by
  sorry
...
@[category research solved, AMS 52]
theorem erdos_1084_upper_d2 : ∃ c > (0 : ℝ), ∀ n > 0, f 2 n < 3 * n - c * sqrt n := by
  sorry
...
theorem erdos_1084_triangular_optimal_d2 : f 2 (3 * n ^ 2 + 3 * n + 1) = 9 * n ^ 2 + 3 * n := by
  sorry
