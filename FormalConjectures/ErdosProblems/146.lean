...
the Gibbs weight [OpenAI26] fixed numerically, and which builds on the [OpenAI26] Lean development
rather than replacing it. It does not subsume the $r=2$ case: the [OpenAI26] bound optimises the
Hamming radius freely and is not implied by the bound below.
-/
