...
/-! ## Kernel-verifiable modular exponentiation (binary method, fuel=100) -/
...
/-! ## Kernel-verifiable fast-doubling Fibonacci over ZMod (fuel=20) -/
...
-- fibZMod m fuel n = (fib n mod m, fib (n+1) mod m) for n < 2^fuel
...
-- Bridge: fibZMod fst = 0 implies fib n ≡ 0 [MOD m]
...
/-! ## Precomputed witnesses (kernel-decidable, no native_decide) -/
...
theorem selfridge_conjecture.variants.exist_pseudo_counterexample :
  ...
  ⟨6601,
   ⟨by norm_num [Nat.odd_iff], Or.inl (by norm_num [Nat.ModEq]),
    pow2_6600_modeq, fib_6600_modeq⟩,
   by norm_num [show 6601 = 7 * 23 * 41 from by norm_num]⟩
...
theorem selfridge_conjecture.variants.pseudo_counterexample' :
  ...
  ⟨⟨by norm_num [Nat.odd_iff], Or.inr (by norm_num [Nat.ModEq]),
    pow2_30888_modeq, fib_30888_modeq⟩,
   by norm_num [show 30889 = 17 * 23 * 79 from by norm_num],
   by norm_num [Nat.ModEq]⟩
