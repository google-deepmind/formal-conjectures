...
-/
private def IsEquidistributed (a b : ℝ) (s : ℕ → ℝ) : Prop :=
...
-/
private def IsEquidistributedModuloOne (s : ℕ → ℝ) : Prop :=
...
from `x`.
-/
private def IsAccumulationPoint (x : ℝ) (s : ℕ → ℝ) : Prop :=
...
-/
private def isAccumulationPoint_iff_exists_subsequence_tendsto
