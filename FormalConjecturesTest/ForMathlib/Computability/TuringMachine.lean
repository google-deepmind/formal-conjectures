...
instance : alwaysHaltingMachine.IsHalting := by
  ...
  exact ⟨0, by aesop⟩
-- `turing_machine%` cannot currently declare its `State{n}` inductive under the module system,
-- so every use of it fails with `Unknown constant State1` / `State2`. The tests are kept here,
-- commented out, rather than deleted; see
-- https://github.com/google-deepmind/formal-conjectures/issues/4734.
